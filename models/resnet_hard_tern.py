...
__all__ = ['resnet_tern_pruned', 'resnet_tern_hard_prune']
...
class ResNet_imagenet(ResNet):
    def __init__(self,  mode=True, cfg=None, num_classes=1000,
        ...
        self.inplanes = 64
        # self.conv1 = nn.Conv2d(3, cfg[0], kernel_size=3, stride=1, padding=1, bias=False)    # cifar
        self.conv1 = nn.Conv2d(3, cfg[0], kernel_size=7, stride=2, padding=3,     # imagenet
                               bias=False)
        if mode:
            self.bnan1 = BinarizeAttention(self.cfg[0])
        self.bn1 = nn.BatchNorm2d(cfg[0])
        self.relu = nn.ReLU(inplace=True)
...
def resnet_tern_hard_prune(model, mode, depth=50, dataset='cifar10'):
    if dataset=='cifar10':
        cfg = [16]
    ...
    for n, p in list(model.named_parameters()):
        if hasattr(p, 'org'):
            cfg.append(int(torch.sum(torch.abs(p.data))))   # 这里包括了三值的mask
    ...
    oldmodel = resnet_tern_pruned(mode=False, cfg=func, depth=depth, dataset=dataset)
    ...
    newmodel = resnet_tern_pruned(mode=mode, cfg=cfg, depth=depth, dataset=dataset)
    print(newmodel)
    ...
    # for [m0, m1] in zip(model.modules(), newmodel.modules()):
        ...
        if isinstance(m0, nn.BatchNorm2d):
            ...
            if conv_counter in downsample or conv_counter > bn3[-1] \
                ...
                w1 = m0.weight.data[:, idx0.tolist(), :, :].clone()  # 对这一部分要区分正负1
                # w2 = torch.ones((len(idx1.tolist()), w1.shape[1], w1.shape[2], w1.shape[3]))
                # print(end_mask)
                # w1 = torch.mul(w1[idx1.tolist(), :, :, :].clone(), end_mask)
                tmp_mask = np.asarray(end_mask.cpu().numpy())  # 对应卷积乘mask
                # w2 = w1[idx1.tolist(), :, :, :].clone()
                for i in range(len(tmp_mask)):
                    # print(i)
                    w1[i,:,:,:] *= tmp_mask[i]   # 只需要对这部分卷积核操作，后面接bnan层的卷积核，其余不用考虑-1
...
if __name__ == '__main__':
    # model = resnet_tern_pruned(dataset='cifar10', depth=20, mode=True)
    # print(model)
    # print(model.cfg)
    ...
    # checkpoint = '/home/ran/mnt1/Model_compress/BinaryAttention_Pruning/results/resnet_bnat20_2020-02-22_21-54-41/checkpoint.pth.tar'
    # model.load_state_dict(torch.load(checkpoint, map_location='cpu')['state_dict'])
    # print('load success')
    # # print(mydict.keys)
    # newmodel, _, _, _ = resnet_tern_hard_prune(model, False, depth=20, dataset='cifar10')
    #
    ...
    # test cifar100
    #######################
    model = resnet_tern_pruned(dataset='cifar100', depth=50, mode=True)
    print(model)
    print(model.cfg)
    x = torch.rand(1, 3, 32, 32)
    output = model.forward(x)
    ...
    print('load success')
    # print(mydict.keys)
    newmodel, _, _, _ = resnet_tern_hard_prune(model, False, depth=50, dataset='cifar100')
