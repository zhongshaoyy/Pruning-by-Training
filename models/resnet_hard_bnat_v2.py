...
# 是适应cifar10的结构，但是带有bnan层，没有真正去掉，对于是否参与梯度更新存疑
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, cfg, stride=1, downsample=None, mode=True):
        ...
        self.conv2 = conv3x3(cfg[0], planes)    # 这一层先不加
...
def resnet_hard_prune(model, mode, depth=50, dataset='cifar10'):
    ...
    for n, p in list(model.named_parameters()):
        if hasattr(p, 'org'):
            # 跳过第一层
            if first:
                cfg.append(int(p.data.size(0)))
                cfg_mask.append(torch.ones(p.data.shape[0]))
                first = False
            else:
                # p.org.copy_(p.data.clamp_(-1,1))
                cfg.append(int(torch.sum(p.data)))
...
if __name__ == '__main__':
    model = resnet_bnat_pruned(dataset='imagenet', depth=18, mode=True)
    print(model)
    print(model.cfg)
    ...
    # print(mydict.keys)
    newmodel, _, _, _ = resnet_hard_prune(model, False, depth=18, dataset='imagenet')
