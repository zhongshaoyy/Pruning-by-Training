...
def resnet_c100_hard_prune(model, mode, depth=50, dataset='cifar100'):
    cfg = [64]
    cfg_mask = [torch.ones(64)]
    block = model.block
    layer = model.layer
    # first = True   # no bnan1 2020/1/17/16:45
    ...
    # print(downsample)
    # print(bn3)
    oldmodel = resnet_bnat_pruned(mode=False, cfg=default_cfg[str(depth)], depth=depth, dataset=dataset)
