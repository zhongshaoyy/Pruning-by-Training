...
# load float model and then train with binaryattention layers
...
parser.add_argument('--norm', type=bool, default=False,
                    help='whether open binary attention')   # 是否让量化感知层L1
parser.add_argument('--pretrained', default='results/resnet56_2020-01-07_21-08-16/checkpoint.pth.tar',
...
def main():
    ...
    if args.evaluate:
        ...
        logging.info("loading pretrained model '%s'", args.pretrained)
        checkpoint = torch.load(checkpoint_file, map_location='cpu')
        state_dict = checkpoint['state_dict']
        mydict = model.state_dict()
        for k in mydict.keys():
            if k in state_dict:
                mydict[k] = state_dict[k]
        model.load_state_dict(mydict)
        logging.info("loaded pretrained model '%s'",
                     checkpoint_file)
    ...
    if args.pretrained:
        ...
        logging.info('After loading pretrained model: Validation Loss {val_loss:.4f} \t'
