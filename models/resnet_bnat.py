...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bnan1 = BinarizeAttention(planes,1.0)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bnan2 = BinarizeAttention(planes,1.0)
        self.bn2 = nn.BatchNorm2d(planes)
...
def resnet_bnat(**kwargs):
    ...
    if dataset == 'imagenet':
        ...
        depth = depth or 56  # 18     # 20   # 32  #44    # 110
...
if __name__ == '__main__':
    model = resnet_bnat(dataset='cifar100')
    print(model)
    # x = torch.rand(1, 3, 32, 32)
    x = torch.rand(1, 3, 224, 224)
    output = model.forward(x)
    print(output.size)
    # pretrained='../results/resnet56_2020-01-07_21-08-16/checkpoint.pth.tar'
    # checkpoint_file = pretrained
    # print("loading checkpoint '%s'", pretrained)
