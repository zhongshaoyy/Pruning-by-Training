...
def Binarize(x, alpha,is_training):
    ...
    if is_training:
        ...
        return alpha*torch.where(abs(x)<0.6135,torch.full_like(x, 0),torch.full_like(x, 1)) + (1-alpha)*x 
    else:

        return torch.where(abs(x)<0.6135,torch.full_like(x, 0),torch.full_like(x, 1))
...
class BinarizeAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        self.weight.data = Binarize(self.weight.org, self.alpha,self.training)
